...
export const PublicView: React.FC = () => {
  ...
  if (isLocked) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-gray-50 dark:bg-[#0a0a0a] transition-colors duration-300">
      <div className="bg-white dark:bg-gray-950 p-10 rounded-3xl shadow-xl border border-red-100 dark:border-red-900/30 max-w-md w-full">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Vault Temporarily Locked</h1>
        <p className="text-gray-500 dark:text-gray-400 mb-6 font-medium">This vault has been blocked due to multiple community reports. Access will be restored once the review period ends.</p>
        <div className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 p-4 rounded-xl text-sm mb-6 border border-red-100 dark:border-red-900/30 font-bold uppercase tracking-tight">
          Locked until: {new Date(vault.lockedUntil!).toLocaleString()}
        </div>
        <Link to="/" className="text-primary-600 dark:text-primary-400 font-bold hover:underline">Back to Home</Link>
      </div>
    </div>
  );
  ...
  if (!hasAccess) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-gray-50 dark:bg-[#0a0a0a] relative overflow-x-hidden transition-colors duration-300">
        <div className="bg-white dark:bg-gray-950 p-10 rounded-3xl shadow-xl border border-gray-100 dark:border-white/5 max-w-md w-full relative z-10">
          <div className="w-16 h-16 bg-gray-900 dark:bg-gray-800 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-lg">
            ...
            <form onSubmit={handleRequestAccess} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-white/5 dark:text-white rounded-xl focus:ring-2 focus:ring-primary-500 focus:bg-white dark:focus:bg-gray-900 outline-none transition-all font-medium"
              ...
              <button
                type="submit"
                disabled={requesting}
                className="w-full bg-primary-600 hover:bg-primary-700 text-white font-bold py-4 px-4 rounded-xl shadow-lg shadow-primary-100/30 transition-all active:scale-95 flex items-center justify-center gap-2"
              >
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-gray-50 dark:bg-[#0a0a0a] relative overflow-x-hidden transition-colors duration-300">
        <div className="bg-white dark:bg-gray-950 p-10 rounded-3xl shadow-xl border border-gray-100 dark:border-white/5 max-w-md w-full relative z-10">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Password Protected</h1>
          <p className="text-gray-500 dark:text-gray-400 mb-8 font-medium">This vault requires a password to view its contents.</p>
          
          <form onSubmit={handleVerifyPassword} className="space-y-4">
            <div>
              <label className="block text-left text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 ml-1">Enter Vault Password</label>
              <input
                type="password"
                required
                value={passwordInput}
                onChange={(e) => { setPasswordInput(e.target.value); setPasswordError(false); }}
                className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all font-medium dark:text-white dark:placeholder-gray-600 ${passwordError ? 'border-red-300 bg-red-50 dark:bg-red-900/10 dark:border-red-800' : 'border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-white/5 focus:bg-white dark:focus:bg-gray-900'}`}
                placeholder="••••••••"
              />
              {passwordError && (
            ...
            <button
              type="submit"
              className="w-full bg-gray-900 dark:bg-primary-600 hover:bg-black dark:hover:bg-primary-700 text-white font-bold py-4 px-4 rounded-xl shadow-lg transition-all active:scale-95 flex items-center justify-center gap-2"
            >
              <ShieldCheck className="w-5 h-5" />
              Unlock Vault
            </button>
          </form>
          <div className="mt-8 flex items-center justify-center gap-2 text-[10px] font-black text-gray-300 dark:text-gray-600 uppercase tracking-widest">
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfd] dark:bg-[#050505] flex flex-col relative overflow-x-hidden transition-colors duration-500">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 flex-1 w-full">
        {currentFiles.length === 0 ? (
          <div className="text-center py-32 bg-gray-50/50 dark:bg-white/5 rounded-3xl border border-dashed border-gray-200 dark:border-white/10">
            <Box className="text-gray-200 dark:text-gray-700 w-16 h-16 mx-auto mb-4" />
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">No files found</h3>
            <p className="text-gray-500 dark:text-gray-400 text-sm">There are no files in this category.</p>
          </div>
        ) : (
      ...
      <footer className="mt-auto border-t border-gray-100 dark:border-white/5 bg-white/60 dark:bg-[#0a0a0a]/60 backdrop-blur-xl">
        <div className="max-w-5xl mx-auto px-4 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          {/* Left: Branding */}
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 bg-primary-600 dark:bg-primary-500 rounded-lg flex items-center justify-center shadow-md shadow-primary-500/20">
              <ShieldCheck className="w-4 h-4 text-white" />
            </div>
            <div className="flex flex-col">
              <span className="text-[11px] font-black text-gray-900 dark:text-white uppercase tracking-widest leading-none">QR Vault</span>
              <span className="text-[9px] font-bold text-gray-400 dark:text-gray-600 uppercase tracking-[0.15em] leading-none mt-0.5">Secure File Sharing</span>
            </div>
          </div>

          {/* Centre: Security badge */}
          <div className="flex items-center gap-2 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/20 rounded-full">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            <span className="text-[9px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-[0.2em]">End-to-End Encrypted</span>
          </div>

          {/* Right: Links */}
          <div className="flex items-center gap-4">
            <a href="/pricing" className="text-[9px] font-black text-gray-400 dark:text-gray-600 hover:text-primary-600 dark:hover:text-primary-400 uppercase tracking-widest transition-colors">Upgrade</a>
            <span className="w-px h-3 bg-gray-200 dark:bg-white/10" />
            <a href="/" className="text-[9px] font-black text-gray-400 dark:text-gray-600 hover:text-primary-600 dark:hover:text-primary-400 uppercase tracking-widest transition-colors">Create Vault</a>
          </div>
        </div>
      </footer>
            ...
            <form onSubmit={handleReportSubmit} className="flex-1 flex flex-col md:flex-row gap-10 overflow-hidden">
              ...
              <div className="flex-1 flex flex-col justify-between">
                ...
                <div className="space-y-4">
                  ...
                  <button type="submit" disabled={isReporting} className="w-full py-5 text-sm font-black text-white bg-red-600 hover:bg-red-700 rounded-[1.5rem] shadow-xl shadow-red-500/20 dark:shadow-none transition-all active:scale-95 disabled:opacity-50 uppercase tracking-widest">
      ...
      {infoFile && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 dark:bg-black/85 backdrop-blur-sm p-4" onClick={() => setInfoFile(null)}>
          <div className="bg-white dark:bg-gray-950 rounded-[2.5rem] p-10 w-full max-w-sm shadow-2xl border border-gray-100 dark:border-white/5 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">Asset Info</h3>
              <button onClick={() => setInfoFile(null)} className="p-2 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-400 dark:text-gray-500 transition-colors"><X className="w-6 h-6" /></button>
            </div>
            <div className="flex flex-col items-center text-center mb-8">
              <div className="w-24 h-24 bg-gray-50 dark:bg-white/5 rounded-[2rem] flex items-center justify-center text-gray-900 dark:text-white mb-4 shadow-inner border border-gray-100 dark:border-white/5">
              ...
              <p className="font-black text-gray-900 dark:text-white text-lg leading-tight uppercase tracking-tight mb-1">{infoFile.name}</p>
              <p className="text-[10px] font-black text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 px-3 py-1 rounded-full uppercase tracking-widest">{infoFile.type}</p>
            </div>
            <div className="space-y-4 bg-gray-50/50 dark:bg-white/5 p-6 rounded-[1.5rem] border border-gray-100 dark:border-white/5 mb-8">
              <div className="flex justify-between text-[11px] font-bold uppercase tracking-wider"><span className="text-gray-400 dark:text-gray-500">Total Size</span><span className="text-gray-900 dark:text-white">{formatBytes(infoFile.size)}</span></div>
              <div className="flex justify-between text-[11px] font-bold uppercase tracking-wider"><span className="text-gray-400 dark:text-gray-500">Format</span><span className="text-gray-900 dark:text-white truncate max-w-[120px]">{infoFile.mimeType}</span></div>
            </div>
            {infoFile.type !== FileType.LINK ? (
              <button onClick={(e) => { handleSingleDownload(infoFile!, e); setInfoFile(null); }} className="w-full bg-gray-900 dark:bg-primary-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-gray-200 dark:shadow-none hover:bg-gray-800 dark:hover:bg-primary-700 transition-all active:scale-95 flex items-center justify-center gap-2"><Download className="w-4 h-4" /> Download</button>
            ) : (
              <a href={infoFile.url} target="_blank" rel="noreferrer" className="w-full bg-primary-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-primary-100/30 hover:bg-primary-700 transition-all active:scale-95 flex items-center justify-center gap-2"><ExternalLink className="w-4 h-4" /> Open Link</a>
            )}
          </div>
        </div>
      )}
