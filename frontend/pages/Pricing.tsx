...
const planData = [
  ...
  },
  {
    ...
    features: [
      ...
      'Up to 72 Hours Vault Expiry',
      '65 & 125 Scan capacity limits',
  ...
  },
  {
    ...
    features: [
      ...
      'Custom Scan capacity limits',
      'Password Protected Vaults',
      'Custom Branding & Logo',
      '24/7 Premium Support'
    ],
    highlight: 'For power users needing absolute control.',
    cta: 'Upgrade to Pro',
    primary: false,
...
export const Pricing = () => {
  ...
  const [appUser, setAppUser] = React.useState<any>(null);
  ...
  const getButtonState = (planId: PlanType) => {
    if (!appUser) return { disabled: false, isCurrentPlan: false, showExpiry: false, needsCancel: false };

    const currentPlanName = appUser.plan === PlanType.STARTER ? 'Plus' : appUser.plan;
    
    if (appUser.plan === planId) {
      return { disabled: true, isCurrentPlan: true, showExpiry: planId !== PlanType.FREE, needsCancel: false };
    }
    if (appUser.plan === PlanType.PRO && planId === PlanType.STARTER) {
  ...
  return (
    <>
      <div className="bg-gray-50 dark:bg-[#0a0a0a] min-h-screen py-16 sm:py-24 relative overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16 sm:mb-20">
          <h2 className="text-xs font-black text-primary-600 dark:text-primary-400 uppercase tracking-[0.25em] mb-4">Pricing Architecture</h2>
          <h1 className="text-4xl sm:text-6xl font-black text-gray-900 dark:text-white tracking-tighter mb-6 uppercase italic">
            Simple Plans.<br/>Transparent Limits.
          </h1>
                ...
                <div className="p-8 sm:p-10">
                  ...
                  <ul className="space-y-4 mb-10">
                    {plan.features.map((feature) => {
                      const isNegative = plan.id === PlanType.FREE && (
                        feature.toLowerCase().includes('delete') || 
                        feature.toLowerCase().includes('remove') || 
                        feature.toLowerCase().includes('limit') ||
                        feature.toLowerCase().includes('no ')
                      );
                      return (
                        <li key={feature} className="flex items-start gap-3">
                          <div className={`mt-1 flex-shrink-0 p-0.5 rounded-full ${
                            isNegative ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-primary-50 dark:bg-primary-900/30'
                          }`}>
                            {isNegative ? (
                          ...
                          <span className={`text-sm leading-tight ${
                            isNegative ? 'text-amber-700 dark:text-amber-400 font-semibold' : 'text-gray-600 dark:text-gray-300'
                          }`}>
                            {feature}
                          </span>
                        </li>
      ...
      <CancelSubscriptionModal
        ...
        onCancelSuccess={() => {
          if (appUser) {
            setAppUser({ ...appUser, plan: PlanType.FREE, subscriptionExpiryDate: undefined });
          }
          setPlanExpiry(null);
        }}
      />
