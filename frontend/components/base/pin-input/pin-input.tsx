...
const Slot = ({ index, className, ...props }: ComponentPropsWithRef<"div"> & { index: number }) => {
    ...
    return (
        <div
            ...
            className={cx(
                "relative flex items-center justify-center rounded-xl bg-gray-50 dark:bg-white/5 text-center text-gray-400 dark:text-gray-500 shadow-xs ring-1 ring-gray-200 dark:ring-white/10 transition-[box-shadow,background-color] duration-100 ease-linear ring-inset",
                sizes[size],
                isFocused && slot?.isActive && "ring-2 ring-primary-500 outline-2 outline-offset-2 outline-primary-500",
                slot?.char && "text-gray-900 dark:text-white ring-2 ring-primary-500",
                disabled && "bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-600 ring-gray-200 dark:ring-gray-700",
                className,
            )}
        >
...
const FakeCaret = ({ size = "md" }: { size?: "sm" | "md" | "lg" }) => {
    return (
        <div
            className={cx(
                "pointer-events-none h-[1em] w-0.5 animate-caret-blink bg-primary-500 dark:bg-white",
...
const Separator = (props: ComponentPropsWithRef<"p">) => {
    return (
        <div role="separator" {...props} className={cx("text-center text-display-xl font-medium text-gray-400 dark:text-gray-500", props.className)}>
            -
        </div>
    );
};
