...
// ─── Hardcoded fallbacks (baked in at build time via .env.production) ──────────
const FALLBACK_SUPABASE_URL  = import.meta.env.VITE_SUPABASE_URL  || 'https://bftzcoofkitmjxfvqdei.supabase.co';
const FALLBACK_SUPABASE_KEY  = import.meta.env.VITE_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJmdHpjb29ma2l0bWp4ZnZxZGVpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAxOTQyNTUsImV4cCI6MjA4NTc3MDI1NX0.Jy-mThUYd92BpnXTjL3BypckXhT8xWKUtr0g5oqH0NU';
const FALLBACK_RAZORPAY_KEY  = import.meta.env.VITE_RAZORPAY_KEY_ID || 'rzp_live_SEru7tZ3y0x3sl';

// ─── Initialize IMMEDIATELY (synchronously) so the client is NEVER null ───────
let activeSupabaseClient: any = createClient(FALLBACK_SUPABASE_URL, FALLBACK_SUPABASE_KEY);
  ...
  if (!initPromise) {
    initPromise = (async () => {
      try {
        ...
        // Upgrade to server-provided config (may differ from fallback)
        activeSupabaseClient = createClient(config.supabaseUrl, config.supabaseAnonKey);
        razorpayKeyId = config.razorpayKeyId;
        isInitialized = true;
      } catch (err) {
        console.warn('Using fallback Supabase config (backend unreachable):', err);
        // Already initialized synchronously above — nothing more to do
        isInitialized = true;
      }
    })();
  }
...
// Proxied client for Auth only
const realSupabase = new Proxy({} as any, {
  get(target, prop) {
    if (!activeSupabaseClient) {
      if (prop === 'auth') {
        return {
          ...
          signInWithPassword: async () => ({ data: {}, error: new Error("Auth not initialized yet") }),
          signUp: async () => ({ data: {}, error: new Error("Auth not initialized yet") }),
          signInWithOAuth: async () => ({ data: {}, error: new Error("Auth not initialized yet") }),
          signOut: async () => ({ error: null }),
        };
      }
      return (target as any)[prop];
    }
    return activeSupabaseClient[prop];
  }
}) as any;
