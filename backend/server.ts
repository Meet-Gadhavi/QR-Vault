...
import fs from 'fs';
import { ensureMazewayTables } from './services/mazewaySchema.js';
...
const allowedOrigins = [
  ...
  'http://localhost:5173',
  'http://localhost:3000',
  'https://glowing-cocada-eca2b3.netlify.app',
  'https://qr-vault-63.netlify.app',
...
apiRouter.get('/config', (req: Request, res: Response) => {
  res.json({
    supabaseUrl: 'https://bftzcoofkitmjxfvqdei.supabase.co',
...
const authenticateUser = async (req: any, res: Response, next: any) => {
  try {
    ...
    let { data: profile } = await realSupabase
      .from('profiles')
      .select('plan')
      .eq('id', user.id)
      .single();
    ...
    if (!profile) {
      // Create profile record on the fly in Supabase!
      ...
      await realSupabase.from('profiles').insert(defaultProfile);
      profile = { plan: 'FREE' };
    }
...
const handleDriveError = (res: Response, error: any, defaultMsg: string) => {
  console.error(`[Google Drive Error] ${defaultMsg}:`, error);
  const isAuthError = 
  ...
  if (isAuthError) {
    return res.status(401).json({ error: 'Google Drive token expired or invalid', details: error.message, authExpired: true });
  }
...
apiRouter.post(['/google-drive/list', '/google-drive/list/'], authenticateUser, async (req: any, res: Response) => {
  ...
  try {
    ...
    handleDriveError(res, error, 'Failed to list files');
  }
});
...
apiRouter.post(['/google-drive/storage-usage', '/google-drive/storage-usage/'], authenticateUser, async (req: any, res: Response) => {
  ...
  try {
    ...
    handleDriveError(res, error, 'Failed to get storage usage');
  }
});
...
apiRouter.post(['/google-drive/ensure-folder', '/google-drive/ensure-folder/'], authenticateUser, async (req: any, res: Response) => {
  ...
  try {
    ...
    handleDriveError(res, error, 'Failed to ensure QRVM folder');
  }
});
...
apiRouter.post(['/google-drive/upload-file', '/google-drive/upload-file/'], authenticateUser, validateUploadLimit, upload.single('file'), async (req: any, res: Response) => {
  ...
  try {
    ...
    handleDriveError(res, error, 'Failed to upload file to Drive');
  }
});
...
apiRouter.post(['/google-drive/save-vault', '/google-drive/save-vault/'], authenticateUser, async (req: Request, res: Response) => {
  ...
  try {
    ...
    handleDriveError(res, error, 'Failed to save vault to Drive');
  }
});
...
apiRouter.post('/google-drive/delete-vault', authenticateUser, async (req: any, res: Response) => {
  ...
  try {
    ...
    handleDriveError(res, error, 'Failed to delete vault folder from Drive');
  }
});
...
// Changelog Reader
apiRouter.get('/changelog', async (req: Request, res: Response) => {
  try {
    let changelogDir = path.join(process.cwd(), 'changelog');
    console.log('[DEBUG] process.cwd():', process.cwd());
    console.log('[DEBUG] First check:', changelogDir, 'exists:', fs.existsSync(changelogDir));
    if (!fs.existsSync(changelogDir)) {
      changelogDir = path.join(process.cwd(), '..', 'changelog');
      console.log('[DEBUG] Fallback check:', changelogDir, 'exists:', fs.existsSync(changelogDir));
    }
    if (!fs.existsSync(changelogDir)) {
      res.json([]);
      return;
    ...
    res.status(500).json({ error: 'Failed to read changelog directory' });
  }
});
  ...
  try {
    ...
    if (generated_signature === razorpay_signature) {
      ...
      if (plan && userId) {
        ...
        const { error: upgradeError } = await realSupabase
          .from('profiles')
          .update({ 
            plan: plan, 
        ...
        await realSupabase.from('invoices').insert({
          id: invoiceId,
          user_id: userId,
...
adminRouter.get('/overview', async (req, res) => {
  try {
    // 1. Fetch Users Data
    const { data: profiles, error: pError } = await realSupabase
      .from('profiles')
      .select('plan');
    ...
    const paidUsersCount = profiles ? profiles.filter((p: any) => p.plan !== 'FREE').length : 0;
    const plans = {
      free: profiles ? profiles.filter((p: any) => p.plan === 'FREE').length : 0,
      starter: profiles ? profiles.filter((p: any) => p.plan === 'STARTER').length : 0,
      pro: profiles ? profiles.filter((p: any) => p.plan === 'PRO').length : 0
    };

    // 2. Fetch Revenue Data (from invoices)
    const { data: invoices, error: iError } = await realSupabase
      .from('invoices')
    ...
    // Definition: Users who have created a vault or whose profile exists
    const { count: realActiveCount } = await realSupabase
      .from('profiles')
...
adminRouter.get('/transactions', async (req, res) => {
  try {
    const { data: invoices, error } = await realSupabase
      .from('invoices')
      .select('*')
    ...
    console.error('[Admin API] Error fetching all invoices:', error);
    res.status(500).json({ error: error.message });
  }
});
...
adminRouter.get('/users', async (req, res) => {
  try {
    // 1. Fetch profiles from Supabase (order by email is safe as fallback)
    const { data: users, error } = await realSupabase
      .from('profiles')
      .select('*')
    ...
    // 2. Fetch auth users to get true created_at dates
    let authUsers: any[] = [];
    try {
      const { data, error: authError } = await realSupabase.auth.admin.listUsers();
      if (!authError && data && data.users) {
        authUsers = data.users;
      }
    } catch (aErr) {
      console.warn('[Admin API] Could not list auth users for mapping dates:', aErr);
    }
    ...
    const mappedUsers = (users || []).map((u: any) => {
      ...
      return {
        ...
        status: u.plan !== 'FREE' ? 'Active' : 'Trial',
...
adminRouter.delete('/users/:userId', async (req, res) => {
  try {
    ...
    // 1. Delete from Supabase Auth if possible
    try {
      await realSupabase.auth.admin.deleteUser(userId);
    } catch (authErr) {
      console.warn('[Admin API] Auth user deletion warning:', authErr);
    }

    // 2. Delete user invoices from Supabase
    await realSupabase
      .from('invoices')
      .delete()
      .eq('user_id', userId);

    // 3. Delete user vaults from Mazeway DB (cascades files on Mazeway)
    await supabase
      .from('vaults')
      .delete()
      .eq('user_id', userId);

    // 4. Delete database profile from Supabase
    const { error } = await realSupabase
      .from('profiles')
      .delete()
      .eq('id', userId);
    ...
    res.json({ status: 'success', message: 'User and all related data deleted permanently' });
  } catch (error: any) {
...
adminRouter.get('/messages', async (req, res) => {
  try {
    const { data: messages, error } = await realSupabase
      .from('contact_messages')
      .select('*')
    ...
    console.error('[Admin API] Error fetching contact messages:', error);
    res.status(500).json({ error: error.message });
  }
});

adminRouter.patch('/messages/:messageId', async (req, res) => {
  try {
    ...
    if (typeof is_replied === 'boolean') updatePayload.is_replied = is_replied;
    ...
    if (error) throw error;
    res.json({ status: 'success', message: 'Message status updated successfully' });
  } catch (error: any) {
    console.error('[Admin API] Error updating contact message status:', error);
    res.status(500).json({ error: error.message });
  }
});
...
adminRouter.delete('/messages/:messageId', async (req, res) => {
  try {
    ...
    console.error('[Admin API] Error deleting contact message:', error);
    res.status(500).json({ error: error.message });
  }
});
